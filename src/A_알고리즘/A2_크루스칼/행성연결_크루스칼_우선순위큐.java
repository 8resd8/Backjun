...
//        adj = new ArrayList<>();
        ...
        // ArrayList -> PriorityQueue버전
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if (map[i][j] != 0) pq.add(new Edge(i, j, map[i][j]));
            }
        }
...
//        }
        ...
        while (!pq.isEmpty()) {
            Edge cur = pq.poll();
            if (union(cur.start, cur.end)) cost += cur.cost;
        }
